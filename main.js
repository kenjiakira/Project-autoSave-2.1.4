...
function onBot() {
  ...
  login(loginData, async (loginError, loginApiData) => {
    ...
    global.config.version = config.version,
      (async () => {
        const commandsPath = `${global.client.mainPath}/module/commands`;
      ...
      (async () => {
        const events = readdirSync(join(global.client.mainPath, 'module/events')).filter(ev => ev.endsWith('.js') && !global.config.eventDisabled.includes(ev));
        console.log(cv(`\n` + `──LOADING EVENTS─●`));
        for (const ev of events) {
          try {
            const event = require(join(global.client.mainPath, 'module/events', ev));
